...
import styles from "../../../../assets/styles/OPSTDIDmr.module.css";
...
const OPSTDIDmr = () => {
  ...
  const fetchDmrs = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/tdi-dmrs`);
  ...
  const exportToExcel = (data) => {
    const formattedData = data.map((item, index) => ({
      ...
      "Source - First Destination - Second Destination": item.route_name,
    }));
  ...
  const handleAddDmr = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(`${API_BASE_URL}/insert-tdi-dmr`, {
        rows,
      });
  ...
  const handleEditDmr = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `${API_BASE_URL}/update-dmr/${encodeURIComponent(dmrsData.waybill)}`,
        { rows }
      );
  ...
  const handleDeleteDmr = async () => {
    ...
    try {
      const response = await axios.delete(
        `${API_BASE_URL}/delete-tdi-dmr/${encodeURIComponent(
          selectedDmr.waybill
        )}`
      );
      ...
      .then((response) => {
        if (response.data.success && Array.isArray(response.data.data)) {
          const options = response.data.data
            .filter((customer) => customer.principal === "TDI")
            .map((customer) => ({
  ...
  return (
    <div className={styles["dmr-content"]}>
      <div className={styles["content-header-container"]}>
        <h1 className={styles["page-title"]}>TDI DMR</h1>
      </div>
          ...
          <table className={styles.table}>
            <thead className={styles.thead}>
              <tr className={styles.htr}>
                ...
                <th className={styles.th}>GP Number</th>
              ...
              {paginatedDmrs.map((dmr, index) => {
                ...
                return (
                  <tr className={styles.btr} key={index}>
                    ...
                    <td className={styles.td}>{dmr.gp_number}</td>
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              ...
                              <input
                                className={`${styles.input} ${
                                  errors.gpNumber ? styles["error-input"] : ""
                                }`}
                                type="number"
                                id="gp_number"
                                value={inv.gpNumber}
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              ...
                              <input
                                className={`${styles.input} ${
                                  errors[`seal_number${custIndex}`]
                                    ? styles["error-input"]
                                    : ""
                                }`}
                                type="text"
                                id="seal_number"
                                value={inv.seal_number}
                                onChange={(e) =>
                                  handleInvoiceFieldChange(
                                    custIndex,
                                    invIndex,
