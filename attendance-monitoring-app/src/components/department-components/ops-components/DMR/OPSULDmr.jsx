...
import styles from "../../../../assets/styles/OPSULDmr.module.css";
...
const OPSULDmr = () => {
  ...
  const fetchDmrs = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/ul-dmrs`);
  ...
  const exportToExcel = (data) => {
    const formattedData = data.map((item, index) => ({
      ...
      Amount: item.amount,
      CBM: item.billing_cbm,
      "Billing CBM": item.billing_cbm,
  ...
  const handleAddDmr = async (e) => {
    ...
    try {
      ...
      const rows = customers.flatMap((ci) =>
        (ci.invoices ?? []).map((inv) => {
          ...
          return {
            ...
            do_number: inv.doNumber || null,
            invoice: inv.invoice || null,
            quantity: inv.quantity || null,
            amount: inv.amount || null,
            cbm: inv.cbm || null,
            billing_cbm: inv.billingCbm || null,
      ...
      const response = await axios.post(`${API_BASE_URL}/insert-ul-dmr`, {
        rows,
      });
  ...
  const handleEditDmr = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `${API_BASE_URL}/update-ul-dmr/${encodeURIComponent(dmrsData.waybill)}`,
        { rows }
      );
  ...
  const handleDeleteDmr = async () => {
    ...
    try {
      const response = await axios.delete(
        `${API_BASE_URL}/delete-ul-dmr/${encodeURIComponent(
          selectedDmr.waybill
        )}`
      );
      ...
      .then((response) => {
        if (response.data.success && Array.isArray(response.data.data)) {
          const options = response.data.data
            .filter((customer) => customer.principal === "UL")
            .map((customer) => ({
  ...
  return (
    <div className={styles["dmr-content"]}>
      <div className={styles["content-header-container"]}>
        <h1 className={styles["page-title"]}>UL DMR</h1>
      </div>
          ...
          <table className={styles.table}>
            <thead className={styles.thead}>
              <tr className={styles.htr}>
                ...
                <th className={styles.th}>DO Number</th>
                <th className={styles.th}>Invoice</th>
                <th className={styles.th}>Quantity</th>
                <th className={styles.th}>Amount</th>
                <th className={styles.th}>CBM</th>
                <th className={styles.th}>Billing CBM</th>
              ...
              {paginatedDmrs.map((dmr, index) => {
                ...
                return (
                  <tr className={styles.btr} key={index}>
                    ...
                    <td className={styles.td}>{dmr.do_number}</td>
                    ...
                    <td className={styles.td}>{dmr.billing_cbm}</td>
                    <td className={styles.td}>
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              ...
                              <input
                                className={`${styles.input} ${
                                  errors.doNumber ? styles["error-input"] : ""
                                }`}
                                type="text"
                                id="do_number"
                                value={inv.doNumber}
                            ...
                            >
                              ...
                              <input
                                className={`${styles.input} ${
                                  errors.billingCbm ? styles["error-input"] : ""
                                }`}
                                type="text"
                                id="billing_cbm"
                                value={inv.billingCbm}
                                onChange={(e) =>
                                  handleInvoiceFieldChange(
                                    custIndex,
                                    invIndex,
