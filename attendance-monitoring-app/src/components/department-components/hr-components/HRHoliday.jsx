...
import styles from "../../../assets/styles/HRHoliday.module.css";
...
const HRHoliday = () => {
  ...
  const filteredHolidays = holidays
    .filter((holiday) => {
      const driverName = (holiday.holiday_name || "").toLowerCase();
      const matchesSearch = driverName.includes(search.toLowerCase());

      const holidayDate = holiday.created_at
        ? new Date(holiday.created_at.split("T")[0])
        : new Date();
      const iswithinDateRange =
        (!appliedFromDate || holidayDate >= new Date(appliedFromDate)) &&
        (!appliedToDate || holidayDate <= new Date(appliedToDate));
    ...
    .sort((a, b) => {
      ...
      if (sortOrder === "holiday_name-asc")
        return a.holiday_name.localeCompare(b.holiday_name);
      if (sortOrder === "holiday_name-desc")
  ...
  const exportToExcel = (data) => {
    const formattedData = data.map((item, index) => ({
      ID: index + 1,
      "Holiday Name": item.holiday_name,
      "Holiday Date": item.holiday_date,
      "Holiday Type": item.holiday_type,
    ...
    saveAs(blob, "holidays.xlsx");
  };

  const exportToPDF = (data) => {
    const doc = new jsPDF();

    doc.text("Holidays Report", 14, 10);
  ...
  const handleExport = () => {
    ...
    const filteredData = holidays.filter((holidays) => {
      const holidaysDate = holidays.created_at
        ? new Date(holidays.created_at.split("T")[0])
        : new Date();
      return (
        (!exportFromDate || holidaysDate >= new Date(exportFromDate)) &&
        (!exportToDate || holidaysDate <= new Date(exportToDate))
      );
    });
  ...
  const dateOptions = useMemo(() => {
    ...
    if (holidaysData.holidayMonth === 2) {
      daysInMonth = 29;
    } else if ([4, 6, 9, 11].includes(holidaysData.holidayMonth)) {
      daysInMonth = 30;
    }
  ...
  const handleAddDevice = async (e) => {
    ...
    if (!holidaysData.holidayName.trim()) {
      setErrors((prev) => ({
        ...prev,
    ...
    if (!holidaysData.holidayMonth && !holidaysData.holidayDate) {
      setErrors((prev) => ({
        ...prev,
        holidayMonth: "Holiday month is required",
        holidayDate: "Holiday date is required.",
      }));
      hasError = true;
    }
    if (!holidaysData.holidayType.trim()) {
      setErrors((prev) => ({
        ...prev,
    ...
    try {
      const response = await axios.post(`${API_BASE_URL}/insert-holiday`, {
        holiday_name: holidaysData.holidayName,
        holiday_month: holidaysData.holidayMonth,
        holiday_date: holidaysData.holidayDate,
        holiday_type: holidaysData.holidayType,
      });
      if (response.data.success) {
      ...
      setErrors((prev) => ({
        ...prev,
        apiError: error.response?.data?.message === "Failed to add holiday.",
      }));
    }
  };

  const toggleEditModal = (holiday = null) => {
    setSelectedDevice(holiday);
    setHolidaysData({
      holidayName: holiday?.holiday_name || "",
      holidayMonth: holiday?.holiday_month || "",
  ...
  const handleEditDevice = async (e) => {
    ...
    if (!selectedDevice || !selectedDevice.id) {
      setErrors((prev) => ({
        ...prev,
        apiError: "Invalid holiday selected.",
      }));
      return;
    }
    ...
    try {
      const response = await axios.put(
        `${API_BASE_URL}/update-holiday/${selectedDevice.id}`,
        {
          holiday_name: holidaysData.holidayName,
          holiday_month: holidaysData.holidayMonth,
          holiday_date: holidaysData.holidayDate,
      ...
      setErrors((prev) => ({
        ...prev,
        apiError:
          error.response?.data?.message === "Failed to update holidayk .",
      }));
    }
  };

  const toggleDeleteModal = (holiday = null) => {
    setSelectedDevice(holiday);
  ...
  const handleDeleteDevice = async () => {
    ...
    try {
      const response = await axios.delete(
        `${API_BASE_URL}/delete-holiday/${selectedDevice.id}`
      );

      if (response.data.success) {
        setHolidays((prevHolidays) =>
          prevHolidays.filter((holiday) => holiday.id !== selectedDevice.id)
        );
        fetchHolidays();
        toggleDeleteModal();
      }
  ...
  return (
    <div className={styles["holiday-content"]}>
      <div className={styles["content-header-container"]}>
        <h1 className={styles["page-title"]}>Holiday</h1>
      </div>
      <div className={styles["content-body-container"]}>
        <div className={styles["add-holiday-button-container"]}>
          <button
            className={styles["add-holiday-button"]}
                    ...
                    <div className={styles["sort-input-container"]}>
                      ...
                      <label className={styles["sort-label"]} htmlFor="a-z">
                        <input
                          ...
                          type="radio"
                          name="sort"
                          checked={tempSortOrder === "holiday_name-asc"}
                          onChange={() => setTempSortOrder("holiday_name-asc")}
                        />
                        A-Z
                      </label>
                      <label className={styles["sort-label"]} htmlFor="z-a">
                        <input
                          ...
                          type="radio"
                          name="sort"
                          checked={tempSortOrder === "holiday_name-desc"}
                          onChange={() => setTempSortOrder("holiday_name-desc")}
                        />
                        Z-A
                      </label>
                    </div>
            ...
            <tbody className={styles.tbody}>
              {paginatedHolidays.map((holiday, index) => (
                <tr className={styles.btr} key={index}>
                  ...
                  <td className={styles.td}>{holiday.exact_date}</td>
                  <td className={styles.td}>{holiday.holiday_type}</td>
                  <td className={styles.td}>
                    <div className={styles["action-container"]}>
                      ...
                      <button
                        ...
                        onClick={() => toggleDeleteModal(holiday)}
                      >
                        <img
        ...
        <Modal isOpen={isAddModalOpen} onClose={toggleAddModal}>
          ...
          >
            <div className={styles["modal-header-container"]}>
              <h3 className={styles["modal-title"]}>Add Holiday</h3>
              <button
            ...
            >
              <label className={styles.label} htmlFor="holiday_name">
                Holiday Name
                <input
                  className={`${styles.input} ${
                    errors.holidayName ? styles["error-input"] : ""
                  }`}
                  type="text"
                  id="holiday_name"
                  name="holidayName"
                  value={holidaysData.holidayName}
              ...
              <div className={styles["modal-holiday-date-container"]}>
                <label className={styles.label} htmlFor="holiday_month">
                  Month
                  <Select
                    className={`${styles.input} ${
                      errors.holidayMonth ? styles["error-input"] : ""
                    }`}
                    styles={{
                      control: (base, state) => ({
                        ...base,
                    ...
                    value={monthOptions.find(
                      (option) => option.value === holidaysData.holidayMonth
                    )}
                ...
                <label className={styles.label} htmlFor="holiday_date">
                  Day
                  <Select
                    className={`${styles.input} ${
                      errors.holidayDate ? styles["error-input"] : ""
                    }`}
                    styles={{
                      control: (base, state) => ({
                        ...base,
                    ...
                    name="holidayDate"
                    id="holiday_date"
                    isDisabled={!holidaysData.holidayMonth}
                    value={dateOptions.find(
                      (option) => option.value === holidaysData.holidayDate
                    )}
                    onChange={(selectedOption) =>
                      handleInputChange(
                        null,
              ...
              <label className={styles.label} htmlFor="holiday_type">
                Type
                <Select
                  className={`${styles.input} ${
                    errors.holidayType ? styles["error-input"] : ""
                  }`}
                  styles={{
                    control: (base, state) => ({
                      ...base,
                  ...
                  value={typeOptions.find(
                    (option) => option.value === holidaysData.holidayType
                  )}
                  onChange={(selectedOption) =>
                    handleInputChange(null, "holidayType", selectedOption.value)
                  }
                />
                {errors.holidayType && (
                  <p className={styles["error-message"]}>
      ...
      {isEditModalOpen && selectedDevice && (
        ...
        >
          <div className={styles["modal-container"]}>
            <div className={styles["modal-header-container"]}>
              <h3 className={styles["modal-title"]}>Edit Holiday</h3>
              <button
                className={styles["close-modal-button"]}
